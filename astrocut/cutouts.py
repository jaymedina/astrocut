...
def asdf_cut(input_files, coordinates, cutout_size, correct_wcs=False, extension=None,
             single_outfile=True, cutout_prefix="cutout", output_dir='.',
             memory_only=False, verbose=False):
    """
    Takes one or more fits files with the same WCS/pointing, makes the same cutout in each file,
    ...
    if 'asdf' in input_files:

        f = asdf.open(input_files)

        data = f.tree['roman']['data']

        # add necessary keywords
        wcs = f.tree['roman']['meta']['wcs']
        header = wcs.to_fits_sip()

        for k in ['cpdis1', 'cpdis2', 'det2im1', 'det2im2', 'sip']:
            header[k] = 'na'

        coords = coordinates#(1,1)#SkyCoord('5h18m57.5s 29d50m25s')
        cutout = Cutout2D(data, position=coords, wcs=WCS(header), size=cutout_size)

        fits.writeto('example_cutout.fits', data=cutout.data, header=cutout.wcs.to_header(), overwrite=True)

    input_files = 'example_cutout.fits'
